...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("api/v1/sections.php", { headers: { "Accept":"application/json" } })
    ...
    .then(data => {
      ...
      (data.sections || []).forEach(sec => {
        ...
        viewBtn.onclick = () =>
          (location.href = `index.php?route=sections/view&section_id=${sec.id}`);
        ...
        delBtn.onclick = () => {
          if (!confirm(`Delete section "${sec.section_name}"?\nAll students in it will be removed.`)) return;
          fetch("index.php?route=sections/deleteSection", {
            method: "POST",
  ...
  editForm.addEventListener("submit", e => {
    ...
    fetch("index.php?route=sections/update", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    .then(json => {
      ...
      card.querySelector("p").textContent  = `${payload.start_school_year} – ${payload.end_school_year}`;
      editModal.style.display = "none";
    })
    .catch(err => alert(err.message));
  });
