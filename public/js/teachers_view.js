(() => {
  ...
  function setStatus(msg, isError = false, timeout = 3500) {
    ...
    if (timeout > 0)
      setTimeout(() => {
        if (tvStatus) tvStatus.textContent = "";
      }, timeout);
  }
  ...
  function parseTeacherFromRow(row) {
    ...
    // Fallback: read cells by stable class names instead of numeric indexes
    const firstCell = row.querySelector(".td-first");
    const lastCell = row.querySelector(".td-last");
    const userCell = row.querySelector(".td-username");

    return {
      id: row.dataset.id || "",
      first_name: (firstCell && firstCell.textContent.trim()) || "",
      last_name: (lastCell && lastCell.textContent.trim()) || "",
  ...
  function fillDeleteModalWithRow(row) {
    ...
    deleteTeacherName.textContent =
      (t.username && `@${t.username}`) || `ID ${t.id}` || "this teacher";
  ...
  function showDeleteError(msg) {
    if (!deleteError) {
      ...
      deleteConfirmBtn.parentNode.insertBefore(
        deleteError,
        deleteConfirmBtn.nextSibling
      );
    }
  ...
  function loadTableRows() {
    ...
    const tbody =
      document.querySelector("#teachersTbody") ||
      document.querySelector("table.tv-table tbody");
    if (!tbody) {
    ...
    return fetch(url, { credentials: "same-origin" })
      ...
      .catch((err) => {
        ...
        tbody.innerHTML =
          '<tr><td colspan="4" style="padding:18px;text-align:center;color:#a00;">Failed to load data.</td></tr>';
  ...
  function updateDomRow(id, newData) {
    ...
    // Update by class name instead of numeric index
    const firstCell = row.querySelector(".td-first");
    const lastCell = row.querySelector(".td-last");
    const userCell = row.querySelector(".td-username");

    if (firstCell) firstCell.textContent = newData.first_name || "";
    if (lastCell) lastCell.textContent = newData.last_name || "";
    if (userCell) userCell.textContent = newData.username || "";

    // Refresh data-json on the Edit button so future edits read correct data
    const editBtn = row.querySelector(".btn-edit");
    if (editBtn) {
      const obj = {
        id: parseInt(id, 10),
      ...
      try {
        ...
        console.warn("Failed to update data-json for row", id, e);
      }
    }
  }
  ...
  function submitUpdate(formEl) {
    ...
    return fetch("index.php?r=teachers/view", {
      ...
      .then((resp) =>
        resp.text().then((text) => ({ ok: resp.ok, status: resp.status, text }))
      )
      .then((result) => {
        ...
        if (!json || !json.ok) {
          const errMsg =
            json && json.error
              ? json.error
              : "Unexpected server response: " + result.text;
          throw new Error(errMsg);
        }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    loadTableRows().then(() => {
      ...
      if (deleteCloseBtn)
        on(deleteCloseBtn, "click", () => {
          clearError(deleteError);
          hide(deleteModal);
        });
      if (deleteCancelBtn)
        on(deleteCancelBtn, "click", () => {
          clearError(deleteError);
          hide(deleteModal);
        });
    ...
    wireBackdrop(editModal, () => {
      clearError(editError);
      hide(editModal);
    });
    wireBackdrop(deleteModal, () => {
      clearError(deleteError);
      hide(deleteModal);
    });
  });
