...
if (!$given_name && isset($data['student_name'])) {
    // Legacy: split "First [Middle] Last"
...
if (!$birth_sex && isset($data['sex'])) $birth_sex = $data['sex']; // legacy

$validSex = ['Male','Female'];
if ($birth_sex !== '' && !in_array($birth_sex, $validSex, true)) {
    // Normalize common inputs like m/f
    $l = strtolower($birth_sex);
    if ($l === 'm' || $l === 'male')   $birth_sex = 'Male';
    elseif ($l === 'f' || $l === 'female') $birth_sex = 'Female';
    else $birth_sex = ''; // force validation error
}

if ($section_id <= 0 || $given_name === '' || $last_name === '' || $birth_sex === '') {
    http_response_code(422);
    echo json_encode(['success'=>false,'error'=>'Missing fields (section_id, given_name, last_name, birth_sex)']);
    exit;
}
...
$stmt->bind_param("sssis", $given_name, $middle_name, $last_name, $section_id, $birth_sex);
$ok = $stmt->execute();
